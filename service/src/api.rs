...
pub async fn query(
    ...
    let req = match req {
        ...
        Err(e) => {
            ...
            return Err(AppError::ParamError(format!("parse query request error: {}", e)));
        }
    };
    let message_type: MessageType = req.message_type.into();
    let query_result = match storage.query(room_id, req.timestamp, Some(message_type), req.username, req.message) {
        Ok(res) => res.iter().map(|message| match message {
